...
export async function readMintingAndBurning() {
  ...
  for (let blockNumber = startBlock; blockNumber < endBlock; blockNumber += 1) {
    ...
    // processWorkingGroupMint(filterByMethod("MintCapacityChanged", events), report);
    ...
    if (prevIssuance !== totalIssuance) {
      // if (prevIssuance !== actualIssuanceDelta && calculatedDelta !== actualIssuanceDelta) {
      console.log(
        `Block: [${blockNumber}]. Issuance: [${issuance}]. Prev issuance: [${prevIssuance}]. Actual Delta: [${actualIssuanceDelta}]. Calculated Delta: [${calculatedDelta}]. Total Minted: [${totalMinted}]. Total Burned: [${totalBurned}]`
      );
      // }

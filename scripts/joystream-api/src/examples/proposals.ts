...
import { Active, Approved, ExecutionFailed, Proposal, ProposalDetails, ProposalId } from "@joystream/types/proposals";
...
async function main() {
  ...
  for (let id of proposalIds) {
    ...
    const proposerHandle = (await api.query.members.membershipById(proposal.proposerId) as Membership).handle_hash.toString()
    ...
    // if (proposalDetails.type != "RuntimeUpgrade") {
